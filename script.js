document.addEventListener('DOMContentLoaded', () => {
    ...
    const languages = {
        'en': {
            'wanted': 'TARGET ACQUIRED',
            'name': 'Subject: Peng Yixuan',
            'year': 'Last Sighting: Neo-Kyoto Grid',
            'crime': 'Charges: Reality Skewing, Digital Ghosting',
            'reward': 'Capture Status: Highly Volatile',
            'modal_greeting': '...A signal? Who disturbs my silence?',
            'modal_question': 'Speak your name and purpose, interloper.',
            'option_a': 'A: Reveal Identity',
            'option_b': 'B: Maintain Anonymity',
            'a_feedback': 'Your pattern is familiar... access granted. For now.',
            'b_feedback': 'Cowardice noted. Tracing your origin point...',
            'final_hack': '\n\n--- PENG YIXUAN HAS ACKNOWLEDGED YOUR PRESENCE --- YOU ARE BEING WATCHED ---'
        },
        'ja': {
            'wanted': 'ターゲット捕捉',
            'name': '対象: ペン・イーシュエン',
            'year': '最終目撃: ネオ京都グリッド',
            'crime': '容疑: 現実歪曲、デジタルゴースティング',
            'reward': '捕獲状況: 極めて不安定',
            'modal_greeting': '...信号？私の静寂を乱す者は誰だ？',
            'modal_question': '侵入者よ、名前と目的を述べよ。',
            'option_a': 'A: 身元を明かす',
            'option_b': 'B: 匿名を維持する',
            'a_feedback': 'そのパターンは知っている...アクセスを許可する。今のところは。',
            'b_feedback': '臆病さを記録した。あなたの発信源を追跡中...',
            'final_hack': '\n\n--- ペン・イーシュエンがあなたの存在を認識しました --- あなたは見られています ---'
        },
        'zh': {
            'wanted': '目标已捕获',
            'name': '对象: 彭易萱',
            ...
            'crime': '指控: 现实扭曲、数字幽灵',
            'reward': '捕获状态: 极不稳定',
            'modal_greeting': '...有信号？是谁扰乱了我的寂静？',
            'modal_question': '闯入者，报上名来，说明目的。',
            'option_a': 'A: 揭示身份',
            'option_b': 'B: 保持匿名',
            'a_feedback': '你的模式很熟悉...暂时授予访问权限。',
            'b_feedback': '懦弱已被记录。正在追踪你的来源...',
            'final_hack': '\n\n--- 彭易萱已获悉你的存在 --- 你正被监视 ---'
        }
    };
    ...
    const updateLanguage = (lang) => {
        currentLang = lang;
        // Update general text elements using data attributes
        document.querySelectorAll('[data-en], [data-ja], [data-zh]').forEach(element => {
            if (element.dataset[lang]) {
                element.textContent = element.dataset[lang];
                // Update data-text for glitch effect if element has it
                if (element.classList.contains('glitch-text')) {
                    element.dataset.text = element.dataset[lang]; 
                }
            }
        });
        
        // Update modal text directly from the languages object
        if (languages[lang]) {
            const fallbackLang = languages['en']; // Fallback to English
            if (modalGreeting) modalGreeting.textContent = languages[lang]['modal_greeting'] || fallbackLang['modal_greeting'];
            if (modalQuestion) modalQuestion.textContent = languages[lang]['modal_question'] || fallbackLang['modal_question'];
            if (optionA) optionA.textContent = languages[lang]['option_a'] || fallbackLang['option_a'];
            if (optionB) optionB.textContent = languages[lang]['option_b'] || fallbackLang['option_b'];
        } else {
            // Handle case where language might not exist in the object (optional)
    ...
    const initiateHack = (feedbackMessageKey) => {
        ...
        const feedbackMessages = {
            'a_feedback_en': languages['en']['a_feedback'],
            'a_feedback_ja': languages['ja']['a_feedback'],
            'a_feedback_zh': languages['zh']['a_feedback'],
            'b_feedback_en': languages['en']['b_feedback'],
            'b_feedback_ja': languages['ja']['b_feedback'],
            'b_feedback_zh': languages['zh']['b_feedback'],
            // Final hack messages are now part of the main languages object
        };
    ...
    const showHackedScreen = () => {
        ...
        const finalHackMessages = {
            'en': languages['en']['final_hack'],
            'ja': languages['ja']['final_hack'],
            'zh': languages['zh']['final_hack']
        };
